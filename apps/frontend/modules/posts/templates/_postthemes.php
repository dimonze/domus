      ...
      <?php if (count($themes)): ?>
        ...
        <?php foreach ($themes as $theme_id => $theme): ?>
          <?php if (isset($post_themes)): ?>
            <?php if (array_key_exists($theme_id, $post_themes)): ?>
          ...
          <?php $url = str_replace(array_keys($translit_tbl), array_values($translit_tbl), $theme)?>
            <li>
              ...
              <strong><?= link_to($theme, '@posts_by_theme?post_type=' . $post_type . '&theme=' . $url . '&page=1') ?></strong>
              <?php else: ?>
                <?= link_to($theme, '@posts_by_theme?post_type=' . $post_type . '&theme=' . $url . '&page=1') ?>
              <?php endif ?>
            </li>
        <?php endforeach ?>
        </ul>
