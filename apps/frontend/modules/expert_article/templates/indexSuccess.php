        ...
        <?php if (isset($sort_order_author)):?>
          <?php if ($sort_order_author == 'asc'): ?>
            <?= link_to(
              ...
              '@expert_article?sort_order=author',
              array('style' => 'color:#090'))?>
          <?php else: ?>
            <?= link_to(
              'По автору<span class="up-arr">&uarr;</span>',
              '@expert_article?sort_order=author-desc',
              array('style' => 'color:#090'))?>
          <?php endif ?>
        <?php else: ?>
          <?= link_to(
              ...
              <?php foreach($author->LatestPosts as $article): ?>
                <div class="item">
                  ...
                  <h4><?php if(empty($article->slug)): //TODO Remove when slugs for all posts will be updated ?>
                    <?= link_to($article->title, '@expert_article_show?author_id=' . $author->id . '&id=' . $article->id) ?>
                    <?php else: ?>
                    <?= link_to($article->title, '@expert_article_show_slug?author_id=' . $author->id . '&slug=' . $article->slug.'-'.$article->id) ?>
                  <?php endif ?></h4>
                  <?= $article->lid ?>
                </div>
              <?php endforeach ?>
          ...
          <div class="authors-list-item">
            <div class="index-adv-teaser">
              <div class="wrap closer">
                <?php include_partial('banner/experts_any5th') ?>
              </div>
            </div>
          </div>
    <div class="content">
...
<div id="aside">
  ...
  <?php cached_component('expert_article', 'authors', null, $cache_prefix, 1500) ?>
