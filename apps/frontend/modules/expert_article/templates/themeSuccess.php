  ...
  <div class="content">
    ...
    <?php if ($pager->getNbResults()): ?>
      <?php foreach ($pager->getResults() as $article): ?>
        <div class="item">
          <?= image_tag(photo($article->PostAuthor)) ?>
          ...
          <h4><?php if(empty($article->slug)): //TODO Remove when slugs for all posts will be updated ?>
            <?= link_to($article->title, '@expert_article_show?author_id=' . $article->PostAuthor->id . '&id=' . $article->id) ?>
            <?php else: ?>
            <?= link_to($article->title, '@expert_article_show_slug?author_id=' . $article->PostAuthor->id . '&slug=' . $article->slug.'-'.$article->id) ?>
          <?php endif ?></h4>
          <?= $article->lid ?>
