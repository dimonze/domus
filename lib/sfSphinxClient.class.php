 ...
 * @edited(2011)   Ray <alrays@yandex.ru>
 */
class sfSphinxClient
{
  ...
  const VER_COMMAND_SEARCH     = 0x118;
  const VER_COMMAND_EXCERPT    = 0x103;
  ...
  {
    if(is_int($min)) $min = floatval($min);
    if(is_int($max)) $max = floatval($max);
    
     assert ( is_string($attribute) );
     assert ( is_float($min) );
     assert ( is_float($max) ); //WTF? — на вход поступает integer
     assert ( $min<=$max );
  ...
  public function AddQuery($query, $index = '*', $comment = '')
  {
    ...
    foreach ($this->filters as $filter)
    {
      ...
      switch ($filter['type'])
      {
        ...
        default:
	       //assert ( 0 && "internal error: unhandled filter type" ); //TODO: WTF?
      }
