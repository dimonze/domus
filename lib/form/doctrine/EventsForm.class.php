...
class EventsForm extends PostForm
{
  public function configure() {
    ...
    $this->setWidget('event_date', new sfWidgetFormInputText(array('label' => 'Дата события')));
    $this->setWidget('event_place', new sfWidgetFormInputText(array('label' => 'Место проведения')));
    $this->setWidget('event_contact', new sfWidgetFormInputText(array('label' => 'Контактная информация')));

    $this->setValidator('event_date', new sfValidatorString(array(
        'required' => false,
        'max_length' => 255
    )));
    $this->setValidator('event_place', new sfValidatorString(array(
        'required' => false,
        'max_length' => 255
    )));
    $this->setValidator('event_contact', new sfValidatorString(array(
        'required' => false,
        'max_length' => 255
    )));
    ...
    $this->setDefaults(array(
      'in_yandex_rss' => false,
      'in_google_xml' => false,
      'in_rambler_rss'=> false
    ));

    $this->widgetSchema['post_text']->setLabel('Текст события');
    $this->widgetSchema['lid']->setLabel('Лид');    
    $this->widgetSchema->setHelp('on_main', 'Поместить событие на главную страницу');    
    $this->getWidgetSchema()->setPositions(array(
      'created_at', 'main_region_id' ,'region_list', 'section', 'themes_list',
      'title', 'subtitle', 'lid', 'slug', 'post_text', 'source', 'source_url',
      'title_photo', 'title_photo_source', 'title_photo_source_url',
      'signature','event_date', 'event_place', 'event_contact', 'status',
      'is_primary', 'on_main', 'tags',
    ...
    if (!sfContext::getInstance()->getUser()->hasCredential('redactor-events-actions')){
