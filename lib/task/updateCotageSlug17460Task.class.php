...
class updateCotageSlug17460Task extends sfBaseTask
{
  protected function configure()
  {
    ...
    $this->name             = 'updateCotageSlug17460Task';
...
Call it with:
  ...
  [php symfony domus:updateCotageSlug17460Task|INFO]
EOF;
  }
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    $ids = Doctrine::getTable('Lot')->createQuery()
            ->select('id')
            ->andWhere('type = ?')
            ->andWhere('slug IS NULL OR slug NOT LIKE ?')
            ->execute( array('cottage-sale', 'kottedzhnyy-poselok-%'), Doctrine::HYDRATE_SINGLE_SCALAR );
