...
function getArrayValueByStringPath($data, $path, $regexp = '#[^\\[\\]]+#') 
{
  $result = null;
  
  if(preg_match_all($regexp, $path, $matches)) {
    $matches = $matches[0];
    $result = $data;
    
    if(is_object($data)) {
      $result = $data->get(array_shift($matches));
    }
    
    foreach($matches as $key) {
      $result = isset($result[$key]) ? $result[$key] : array();
    }
  }
